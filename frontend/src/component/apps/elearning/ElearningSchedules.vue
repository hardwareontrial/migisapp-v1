  ...
  <div>
    <table-schedule
      ...
      <template #cell(duration)="data">
        {{ data.item.dataquestion.duration }} Menit
      </template>
...
export default {
  ...
  data(){
    return{
      ...
      busy: false,
      fields: [
        { key: 'name', label: 'Nama Ujian', thClass: 'text-left', thStyle: { width: "30%" } },
        { key: 'tipe', label: 'Jenis', thClass: 'text-center', thStyle: { width: "10%" }, tdClass: 'text-center' },
        { key: 'date', label: 'Waktu', thClass: 'text-center', tdClass: 'text-center' },
        { key: 'duration', label: 'Durasi', thClass: 'text-center', tdClass: 'text-center' },
  ...
  methods: {
    ...
    setactive(options){
      ...
      http.post('okm/schedule/setactive/'+options.id, { newactive: newactive })
      .then((res) => {
        this.handleFetchData()
        this.$toast({

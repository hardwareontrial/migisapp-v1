<template>
  <div>
    <material-detail-card background-card-color="LightBlue">
    ...
    <app-collapse accordion type="margin">
      <app-collapse-item title="Materi">
        ...
        <div v-else>File materi kosong.</div>
              ...
              <div>
                ...
                <b-button
                  ...
                  :to="{
                    ...
                    params: { id: qsl.id, slug: qsl.slug, title: qsl.title },
                  }"
                  variant="flat-primary"
                  class="btn-icon"
                  size="sm"
                ...
                <b-button
                  v-if="
                    qsl.scheduled.length <= 0 &&
                    $can('delete', 'AppOKMQuestion')
                  "
                  variant="flat-danger"
                  class="btn-icon"
                  size="sm"
...
export default {
  ...
  data() {
    return {
      ...
      stream:
        "http://" + process.env.VUE_APP_API_ENDPOINT + "/storage/ViewerJS/#",
      datafile: null,
      qstlist: [],
    };
  },
  methods: {
    ...
    editdetailmaterial() {
      this.$router.push({
        name: "apps-elearning-materials-edit",
        params: { id: this.materialdata.id },
      });
    },
    addmaterial() {
  ...
  computed: {
    deptname() {
      ...
      let scoped = this.deptList.filter((item) => {

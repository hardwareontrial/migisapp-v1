<template>
  <div>
    <div>
      <b-table-simple responsive style="width: 100%" class="table">
        <b-thead>
          <b-tr>
        ...
        <b-tbody v-for="(participant, i) in data" :key="i">
          <b-tr>
              ...
              <b-media vertical-align="center">
                <template #aside>
                  <b-avatar
                    ...
                    :src="
                      participant.datauser.avatar
                        ? participant.datauser.avatarlink
                        : null
                    "
                ...
                <small>
                  |
                  {{
                    participant.datauser.position
                      ? participant.datauser.position.name
                      : ""
                  }}
                </small>
              </b-media>
            ...
            <b-td>
              <b-badge style="background-color: CornflowerBlue">
                {{
                  participant.datauser.position
                    ? participant.datauser.position.deptname.name
                    : "-"
                }}
              </b-badge>
            </b-td>
            ...
            <b-td class="text-center">
              <b-dropdown variant="link" no-caret>
                ...
                <b-dropdown-item @click="$emit('infodata', participant)">
                  <feather-icon icon="InfoIcon" />
                  <span class="align-middle ml-50">Detail</span>
                </b-dropdown-item>
                <b-dropdown-item
                  @click="$emit('setfinish', participant)"
                  :disabled="
                    participant.isdone !== 3 && $can('update', 'AppOKMSchedule')
                  "
                >
                  <feather-icon icon="CheckCircleIcon" />
                  <span class="align-middle ml-50">Setel Selesai</span>
                </b-dropdown-item>
                <b-dropdown-item
                  @click="$emit('deletedata', participant.id)"
                  v-if="
                    participant.isdone === 2 && $can('update', 'AppOKMSchedule')
                  "
                >
